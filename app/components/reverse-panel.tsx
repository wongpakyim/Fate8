...
export function ReversePanel({ text, start, end, result, error, onTextChange, onStartChange, onEndChange, onSearch, onApply }: {
  ...
  return <section className="reverse-page module-page" id="reverse-page">
    <div className="detail-heading"><span className="step">反</span><div><h2>八字反排</h2><p>输入四柱八个字，在公元 1000–2100 年间寻找实际匹配的阳历时间</p></div></div>
    <article className="reverse-workbench">
      <div className="modal-heading"><span className="seal">反</span><div><small>REVERSE LOOKUP</small><h2>八字反查出生时刻</h2><p>反查沿用当前经度、时区、真太阳时与换日口径。</p></div></div>
      ...
      {result && <div className="reverse-results"><div className="results-head"><strong>找到 {result.total} 个代表时刻</strong><span>{result.range[0]} — {result.range[1]}</span></div>{result.matches.length ? <div className="match-list">{result.matches.map((match) => <button key={match.solarTime} onClick={() => onApply(match.solarTime)}><span><strong>{match.solarTime}</strong><small>真太阳时 {match.trueSolarTime.slice(11)} · {match.month}</small></span><b>{match.fourPillars}</b><i>采用 →</i></button>)}</div> : <div className="empty-result">没有找到匹配时刻。请检查四柱顺序、时间范围与排盘口径。</div>}<p>{result.notice}</p></div>}
    </article>
  </section>;
}
