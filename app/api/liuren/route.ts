...
function calculate(input: Record<string, unknown>) {
  ...
  return buildSimpleChart(pillars, { liuRen: { monthGeneral: input.monthGeneral }, qiMen: { method: defaults.qiMen.method } }).liuRen;
}

function respond(result: ReturnType<typeof calculate>, format: unknown) {
  if (format === "text" || format === "file") {
    return new Response(formatLiuRenText(result) + "\n", { headers: { ...corsHeaders, "Content-Type": "text/plain; charset=utf-8", ...(format === "file" ? { "Content-Disposition": "attachment; filename=liuren-result.txt" } : {}) } });
  }
...
export async function GET(request: Request) {
  try {
    ...
    if (!input.solarTime && !input.datetime) return Response.json({ name: "知命大六壬 API", module: "liu-ren", usage: "GET /api/liuren?solarTime=1992-03-15%2014:30&longitude=113.27", monthGeneral: "省略时按中气自动换将，也可传 子/神后 等手动覆盖", formats: ["json", "text", "file"] }, { headers: corsHeaders });
    return respond(calculate(input), input.format);
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "六壬排盘失败" }, { status: 400, headers: corsHeaders });
  }
}
