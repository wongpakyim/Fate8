...
function calculate(input: Record<string, unknown>) {
  ...
  return buildSimpleChart(pillars, { liuRen: { monthGeneral: input.monthGeneral }, qiMen: { method: input.method || defaults.qiMen.method } });
}

function respond(result: ReturnType<typeof buildSimpleChart>, format: unknown) {
  if (format === "text" || format === "file") return new Response(formatSimpleChartText(result) + "\n", { headers: { ...corsHeaders, "Content-Type": "text/plain; charset=utf-8", ...(format === "file" ? { "Content-Disposition": "attachment; filename=simple-chart.txt" } : {}) } });
...
export async function GET(request: Request) {
  try {
    ...
    if (!input.solarTime && !input.datetime) return Response.json({ name: "简化排盘 API", module: "simple-chart", usage: "GET /api/simple?solarTime=1992-03-15%2014:30&longitude=113.27", formats: ["json", "text", "file"] }, { headers: corsHeaders });
    return respond(calculate(input), input.format);
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "简化排盘失败" }, { status: 400, headers: corsHeaders });
  }
}
