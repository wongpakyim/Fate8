...
export function calculateZiWei(calculation, options = {}) {
  ...
  return {
    schemaVersion: "1.0.0",
    module: { name: "zi-wei", version: "0.2.0", responsibility: "三合派紫微盘面与中州派安星的可序列化结果" },
    rules: {
      school: "三合派",
      starAlgorithm: "中州派",
      chartType: "天盘",
...
function palaceText(palace, selection) {
  ...
  return [
    ...
    `甲 ${starText(palace.majorStars)}`,
    `乙 ${starText(palace.minorStars)}`,
    `丙 ${starText(palace.adjectiveStars)}`,
    `大限 ${decadeName} ${decadeStars}`,
    `流年 ${yearName} ${yearStars}`,
  ];
}

export function formatZiWeiText(result, decadeIndex, yearValue) {
  ...
  const mutagen = (names) => names.map((name, index) => `${MUTAGEN_LABELS[index]}:${name}`).join(" ");
  ...
  return [
    ...
    `大限 ${selection.decade.ageRange.join("–")}岁 ${selection.decade.yearRange.join("–")}  ${mutagen(selection.decade.mutagen)}`,
    `流年 ${selection.year.year} ${selection.year.age}岁 ${selection.year.heavenlyStem}${selection.year.earthlyBranch}  ${mutagen(selection.year.mutagen)}`,
    ring,
  ].join("\n");
}
