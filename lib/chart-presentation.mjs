...
import { buildReferenceShenSha } from "./metaphysics-core.mjs";
...
const SEASONAL_STATE_BY_MONTH_ORDER = [
  SEASONAL_STATES.spring,
  SEASONAL_STATES.spring,
  SEASONAL_STATES.earth,
  SEASONAL_STATES.summer,
  SEASONAL_STATES.summer,
  SEASONAL_STATES.earth,
  SEASONAL_STATES.autumn,
  SEASONAL_STATES.autumn,
  SEASONAL_STATES.earth,
  SEASONAL_STATES.winter,
  SEASONAL_STATES.winter,
  SEASONAL_STATES.earth,
];
...
function seasonalState(element, monthOrder) {
  return SEASONAL_STATE_BY_MONTH_ORDER[mod(monthOrder, 12)][element];
}
...
const BRANCH_RELATION_PAIRS = Object.freeze({
  harmony: [[0,1],[2,11],[3,10],[4,9],[5,8],[6,7]],
  clash: [[0,6],[1,7],[2,8],[3,9],[4,10],[5,11]],
  harm: [[0,7],[1,6],[2,5],[3,4],[8,11],[9,10]],
  break: [[0,9],[1,4],[2,11],[3,6],[5,8],[7,10]],
});
const BRANCH_PUNISHMENT_TARGETS = [3,10,5,0,4,8,6,1,2,9,7,11];

function pairedBranchTarget(pairs, branchIndex) {
  const pair = pairs.find(([left, right]) => left === branchIndex || right === branchIndex);
  return pair[0] === branchIndex ? pair[1] : pair[0];
}

export function getBranchRelationHints(branchIndex) {
  const index = mod(branchIndex, 12);
  return [
    { type: "合", targetIndex: pairedBranchTarget(BRANCH_RELATION_PAIRS.harmony, index), explanation: "结合整合" },
    { type: "破", targetIndex: pairedBranchTarget(BRANCH_RELATION_PAIRS.break, index), explanation: "破坏/不完整" },
    { type: "刑", targetIndex: BRANCH_PUNISHMENT_TARGETS[index], explanation: "折磨痛苦" },
    { type: "冲", targetIndex: pairedBranchTarget(BRANCH_RELATION_PAIRS.clash, index), explanation: "冲击远离" },
    { type: "害", targetIndex: pairedBranchTarget(BRANCH_RELATION_PAIRS.harm, index), explanation: "妨碍不得其心" },
  ].map((relation) => ({ ...relation, target: BRANCHES[relation.targetIndex].name }));
}
...
function branchRelations(pillars) {
  ...
  for (let a = 0; a < pillars.length; a += 1) for (let b = a + 1; b < pillars.length; b += 1) {
    ...
    if (BRANCH_RELATION_PAIRS.harmony.some((x) => x[0] === pair[0] && x[1] === pair[1])) relations.push(`${pillars[a].label}${pillars[b].label}：${BRANCHES[ba].name}${BRANCHES[bb].name}六合`);
    if (BRANCH_RELATION_PAIRS.clash.some((x) => x[0] === pair[0] && x[1] === pair[1])) relations.push(`${pillars[a].label}${pillars[b].label}：${BRANCHES[ba].name}${BRANCHES[bb].name}相冲`);
    if (BRANCH_RELATION_PAIRS.harm.some((x) => x[0] === pair[0] && x[1] === pair[1])) relations.push(`${pillars[a].label}${pillars[b].label}：${BRANCHES[ba].name}${BRANCHES[bb].name}相害`);
...
function shenShaProfile(calculation, pillars) {
  const reference = calculation.fourPillars.day;
  const byBranch = buildReferenceShenSha({
    yearBranchIndex: calculation.fourPillars.year.branch.index,
    monthBranchIndex: calculation.fourPillars.month.branch.index,
    referenceStemIndex: reference.stem.index,
    referenceBranchIndex: reference.branch.index,
    referencePillarIndex: reference.index,
  });
  const items = pillars.map((pillar) => {
    const palace = byBranch[pillar.branch.index];
    return { label: pillar.label, pillar: pillar.value, stars: palace.stars, groups: palace.groups };
  });
  items.forEach((item, index) => { pillars[index].shenSha = item.stars; pillars[index].shenShaGroups = item.groups; });
  return { byPillar: items, all: [...new Set(items.flatMap((item) => item.stars))], note: "默认以日柱干支为参照，按干煞、支煞、年煞、季煞、月煞与杂煞分类。" };
}
...
export function buildBaziChart(calculation) {
  ...
  const shenSha = shenShaProfile(calculation, pillars);
  return {
...
function normalizeReference(chart, reference = {}) {
  ...
  const shenShaStemIndex = Number.isInteger(reference.shenShaStemIndex) ? mod(reference.shenShaStemIndex, 10) : stemIndex;
  const shenShaBranchIndex = Number.isInteger(reference.shenShaBranchIndex) ? mod(reference.shenShaBranchIndex, 12) : branchIndex;
  const shenShaPillarIndex = Number.isInteger(reference.shenShaPillarIndex) ? mod(reference.shenShaPillarIndex, 60) : dayPillar.index;
  ...
  return {
    ...
    shenShaReference: {
      source: reference.shenShaSource || reference.source || "日柱",
      stem: { ...STEMS[shenShaStemIndex], index: shenShaStemIndex },
      branch: { ...BRANCHES[shenShaBranchIndex], index: shenShaBranchIndex },
      pillarIndex: shenShaPillarIndex,
    },
  };
}

function shenShaMapForReference(chart, reference) {
  return buildReferenceShenSha({
    yearBranchIndex: chart.fourPillars.year.branch.index,
    monthBranchIndex: chart.fourPillars.month.branch.index,
    referenceStemIndex: reference.shenShaReference.stem.index,
    referenceBranchIndex: reference.shenShaReference.branch.index,
    referencePillarIndex: reference.shenShaReference.pillarIndex,
  });
}

function focusPillar(chart, pillar, reference, fallbackLabel, shenShaByBranch) {
  ...
  const shenSha = shenShaByBranch[branchIndex];
  return {
    ...
    shenSha: shenSha.stars,
    shenShaGroups: shenSha.groups,
  };
}
...
export function getBaziFocusView(chart, reference, pillars) {
  ...
  const shenShaByBranch = shenShaMapForReference(chart, normalizedReference);
  return { reference: normalizedReference, pillars: targetPillars.map((pillar, index) => focusPillar(chart, pillar, normalizedReference, `第${index + 1}柱`, shenShaByBranch)) };
}
...
export function getBaziNodeRelations(chart, targetIndex) {
  ...
  const pillars = [chart.fourPillars.year, chart.fourPillars.month, chart.fourPillars.day, chart.fourPillars.hour];
  const targetPillar = pillars[target.pillarIndex];
  return getBaziRelationsByReference(chart, {
    ...
    shenShaStemIndex: targetPillar.stem.index,
    shenShaBranchIndex: targetPillar.branch.index,
    shenShaPillarIndex: targetPillar.index,
    targetIndex,
  });
}
...
export function getBaziRelationsByReference(chart, reference) {
  ...
  return {
    ...
    reference: { ...normalizedReference.stem, source: normalizedReference.source, key: normalizedReference.key, monthStatus: normalizedReference.monthStatus, monthGrowth: normalizedReference.monthGrowth, seatGrowth: normalizedReference.seatGrowth, shenShaReference: normalizedReference.shenShaReference },
    relations: nodes.map((node, index) => {
      ...
      const nodeShenSha = shenShaByBranch[seatBranchIndex].stars;
      if (node.kind === "stem") {
