...
function buildDoorHiddenInstruments(earthInstruments, doorsByPalace) {
  const originalPalaceByDoor = Object.fromEntries(ROTATION_RING.map((palaceNumber) => [QIMEN_PALACES[palaceNumber].originalDoor, palaceNumber]));
  const byPalace = { 5: null };
  const carriers = ROTATION_RING.map((palaceNumber) => {
    const door = doorsByPalace[palaceNumber];
    const originalPalace = originalPalaceByDoor[door];
    const hiddenInstrument = earthInstruments[originalPalace];
    byPalace[palaceNumber] = hiddenInstrument;
    return { palace: palaceNumber, door, originalPalace, hiddenInstrument };
  });

  return {
    method: "八门携带暗干：随门取其元旦盘本宫的地盘奇仪",
    byPalace,
    carriers,
  };
}
...
export function calculateQiMen(calculation, options = {}) {
  ...
  const hiddenStems = buildDoorHiddenInstruments(earthInstruments, doorsByPalace);
  ...
  const palaces = DISPLAY_ORDER.map((palaceNumber) => {
    ...
    return {
      ...
      hiddenInstrument: hiddenStems.byPalace[palaceNumber],
      hiddenGrowth: palaceNumber === 5 ? [] : palaceBranches.map((branch) => ({ stem: hiddenStems.byPalace[palaceNumber], branch, stage: getTwelveGrowthStage(hiddenStems.byPalace[palaceNumber], branch) })),
  ...
  return {
    schemaVersion: "1.0.0",
    module: { name: "qi-men", version: "0.2.2", responsibility: "接收四柱结果并生成时家转盘拆补法奇门盘" },
    ...
    config: { method: "chai-bu", methodName: "拆补法", style: "时家转盘", centerStarLodging: "天禽寄坤二", hiddenStemMethod: hiddenStems.method },
    ...
    notices: [
      ...
      "隐干采用八门携带暗干法：每门携带其元旦盘本宫的地盘奇仪，中宫不设八门隐干。",
