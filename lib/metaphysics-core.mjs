...
export function buildBranchShenSha(calculation) {
  ...
  const referenceBranches = [calculation.fourPillars.year.branch.index, calculation.fourPillars.day.branch.index];
  const stars = Array.from({ length: 12 }, () => new Set());
  const branchRules = [
  ...
  for (const reference of referenceBranches) {
    for (const rule of branchRules) {
      const match = rule.groups.find(([group]) => group.includes(reference));
      if (match) stars[match[1]].add(rule.name);
    }
  }
  const stemRules = [
  ...
  for (const rule of stemRules) for (const target of rule.targets) stars[target].add(rule.name);
  return stars.map((items, index) => ({ branch: { ...BRANCHES[index], index }, stars: [...items] }));
}
