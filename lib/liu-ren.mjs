...
import { BRANCHES, STEMS, mod } from "./four-pillars.mjs";
...
function uniqueUpperGodCount(lessons) {
  return new Set(lessons.map((lesson) => lesson.upper.index)).size;
}

function buildThreeTransmission({ lessons, upperAt, dayStem, dayBranchIndex, plateOffset }) {
  const lessonCount = uniqueUpperGodCount(lessons);
  const lessonPattern = lessonCount >= 4 ? "四课全" : lessonCount === 3 ? "不备" : "两课";
  ...
  const controlRelation = lowerControls.length ? "下贼上" : upperControls.length ? "上克下" : null;
  const finish = (transmission) => ({ ...transmission, lessonCount, lessonPattern, controlRelation });

  const fromDirect = () => {
    const candidates = lowerControls.length ? lowerControls : upperControls;
    if (!candidates.length) return null;
    if (candidates.length === 1) return { initial: candidates[0].upper.index, initialSource: candidates[0].source, method: lowerControls.length ? "重审" : "元首" };
    const chosen = chooseByPolarityThenPosition(candidates, dayStem.polarity);
    return { initial: chosen.lesson.upper.index, initialSource: chosen.lesson.source, method: chosen.method };
  };

  const direct = fromDirect();
  if (plateOffset === 0) {
    const initial = direct?.initial ?? (dayStem.polarity === "阳" ? lessons[0].upper.index : dayBranchIndex);
    const initialSource = direct?.initialSource ?? (dayStem.polarity === "阳" ? "干课" : "支课");
    let middle = PUNISHMENT[initial];
    if (middle === initial) middle = initialSource === "干课" ? lessons[2].upper.index : lessons[0].upper.index;
    let final = PUNISHMENT[middle];
    if (final === middle) final = mod(middle + 6, 12);
    return finish({ initial, middle, final, initialSource, method: direct ? `伏吟·${direct.method}` : "伏吟" });
  }

  if (direct) {
    const prefix = plateOffset === 6 ? "返吟·" : "";
    return finish({ ...direct, method: prefix + direct.method, middle: upperAt(direct.initial), final: upperAt(upperAt(direct.initial)) });
  }

  if (plateOffset === 6) {
    return finish({ initial: HORSE[dayBranchIndex], middle: lessons[2].upper.index, final: lessons[0].upper.index, method: "返吟·井栏" });
  }

  // 九宗门以四课上神是否重复判断课体是否齐备。只有两课时按八专，
  // 无须再走遥克；三课不备仍须先查遥克，无遥克才取别责。
  if (lessonCount <= 2) {
    const initial = dayStem.polarity === "阳" ? mod(lessons[0].upper.index + 2, 12) : mod(lessons[3].upper.index - 2, 12);
    return finish({ initial, middle: lessons[0].upper.index, final: lessons[0].upper.index, method: "八专" });
  }
  ...
  if (remote.length) {
    ...
    return finish({ initial, middle: upperAt(initial), final: upperAt(upperAt(initial)), method: remoteUpper.length ? "遥克·蒿矢" : "遥克·弹射" });
  }
  ...
  if (lessonCount === 3) {
    ...
    return finish({ initial, middle: lessons[0].upper.index, final: lessons[0].upper.index, method: "别责" });
  }
  const initial = dayStem.polarity === "阳" ? upperAt(9) : Array.from({ length: 12 }, (_, index) => index).find((index) => upperAt(index) === 9);
  return finish({ initial, middle: dayStem.polarity === "阳" ? lessons[2].upper.index : lessons[0].upper.index, final: dayStem.polarity === "阳" ? lessons[0].upper.index : lessons[2].upper.index, method: "昴星" });
}
...
export function calculateLiuRen(calculation, options = {}) {
  ...
  const xunStartBranchIndex = mod(xunStartIndex, 12);
  const voidBranchIndexes = [mod(xunStartBranchIndex + 10, 12), mod(xunStartBranchIndex + 11, 12)];
  const dunStemForBranch = (branchIndex) => {
    const offset = mod(branchIndex - xunStartBranchIndex, 12);
    return offset < 10 ? { ...STEMS[offset], index: offset } : null;
  };
  ...
  const earthPlate = Array.from({ length: 12 }, (_, earthIndex) => {
    ...
    const heavenShenSha = core.branchShenSha[heavenIndex];
    const earthShenSha = core.branchShenSha[earthIndex];
    return {
      earth: branch(earthIndex),
      heaven: branch(heavenIndex),
      earthVoid: voidBranchIndexes.includes(earthIndex),
      heavenVoid: voidBranchIndexes.includes(heavenIndex),
      dunStem: dunStemForBranch(heavenIndex),
      heavenlyGeneral,
      heavenlyGeneralDetail: generalDetail(heavenlyGeneral),
      heavenShenSha: heavenShenSha.stars,
      heavenShenShaGroups: heavenShenSha.groups,
      earthShenSha: earthShenSha.stars,
      earthShenShaGroups: earthShenSha.groups,
      // 兼容 0.3 版调用方：未注明层级的神煞仍指地盘宫位神煞。
      shenSha: earthShenSha.stars,
      shenShaGroups: earthShenSha.groups,
    };
  });
  ...
  const makeLesson = (order, label, lower, earthIndex, source) => {
    ...
    return {
      ...
      upperDunStem: dunStemForBranch(upper.index),
      earthDunStem: dunStemForBranch(earthIndex),
  ...
  const threeTransmissions = {
    ...
    lessonCount: rawTransmissions.lessonCount,
    lessonPattern: rawTransmissions.lessonPattern,
    controlRelation: rawTransmissions.controlRelation,
    items: transmissionIndexes.map((index, order) => {
      const heavenlyGeneral = generalForHeaven(index);
      return { order: order + 1, label: ["初传", "中传", "末传"][order], branch: branch(index), dunStem: dunStemForBranch(index), heavenlyGeneral, heavenlyGeneralDetail: generalDetail(heavenlyGeneral), sixRelation: sixRelation(dayStem.element, BRANCHES[index].element) };
    }),
  };

  return {
    schemaVersion: "1.0.0",
    module: { name: "liu-ren", version: "0.5.3", responsibility: "消费公共核心并生成大六壬十二宫、四课、三传、旬遁干、空亡及天地盘神煞" },
    ...
    xunDun: {
      startIndex: xunStartIndex,
      start: `甲${BRANCHES[xunStartBranchIndex].name}`,
      rule: "日旬遁干",
      voidBranchIndexes,
      voidBranches: voidBranchIndexes.map((index) => BRANCHES[index].name),
    },
    earthPlate,
    fourLessons,
    threeTransmissions,
...
function liuRenHeavenCell(cell) {
  const heavenGroupedShenSha = cell.heavenShenShaGroups.map((group) => `${group.category}：${group.items.join("、")}`).join("；");
  const earthGroupedShenSha = cell.earthShenShaGroups.map((group) => `${group.category}：${group.items.join("、")}`).join("；");
  return [
    `上神 ${cell.heaven.name}${cell.heaven.element}`,
    `旬遁 ${cell.dunStem ? `${cell.dunStem.name}${cell.dunStem.element}` : "空"}`,
    `天将 ${cell.heavenlyGeneral}${cell.heavenlyGeneralDetail.element}`,
    `天盘神煞 ${heavenGroupedShenSha || "—"}`,
    `地盘神煞 ${earthGroupedShenSha || "—"}`,
  ];
}
...
export function formatLiuRenText(result) {
  const transmissions = result.threeTransmissions.items.map((item) => `${item.label}${item.dunStem?.name || "空"}${item.branch.name}${item.heavenlyGeneral}${item.sixRelation}`).join(" → ");
  return [
    ...
    `占时      ${result.divinationTime.branch.name}时（${result.divinationTime.pillar}）`, `旬遁      ${result.xunDun.start}旬（空亡 ${result.xunDun.voidBranches.join("、")}）`, `课式      ${result.threeTransmissions.method}`, `三传      ${transmissions}`,
