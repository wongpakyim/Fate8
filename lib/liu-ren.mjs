...
export function calculateLiuRen(calculation, options = {}) {
  ...
  const earthPlate = Array.from({ length: 12 }, (_, earthIndex) => {
    ...
    return { earth: branch(earthIndex), heaven: branch(heavenIndex), heavenlyGeneral, heavenlyGeneralDetail: generalDetail(heavenlyGeneral), shenSha: core.branchShenSha[earthIndex].stars };
  });
  ...
  const makeLesson = (order, label, lower, earthIndex, source) => {
    ...
    return { order, label, source, earthIndex, earthPalace: BRANCHES[earthIndex].name, lower, upper, heavenlyGeneral, heavenlyGeneralDetail: generalDetail(heavenlyGeneral), relation };
  };
...
export function formatLiuRenFourLessonsText(result) {
  const lessonsRightToLeft = [...result.fourLessons].reverse().map((lesson) => [
    lesson.label,
    `天将 ${lesson.heavenlyGeneral}${lesson.heavenlyGeneralDetail.element}`,
    `上神 ${lesson.upper.name}${lesson.upper.element}`,
    `下神 ${lesson.lower.name}${lesson.lower.element}`,
    `关系 ${lesson.relation}`,
  ]);
  return ["四课（第一课在最右，自右向左）", renderTextGrid([lessonsRightToLeft], { cellWidth: 18, align: "center" })].join("\n");
}
