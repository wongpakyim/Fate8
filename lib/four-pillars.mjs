...
export function reverseSearchFourPillars(pillarInput, options = {}) {
  ...
  for (let cycleYear = startYear - 1; cycleYear <= endYear; cycleYear += 1) {
    ...
    for (let cursor = gregorianJdn(firstLocal.year, firstLocal.month, firstLocal.day), last = gregorianJdn(lastLocal.year, lastLocal.month, lastLocal.day); cursor <= last; cursor += 1) {
      ...
      const result = calculateFourPillars({ solarTime: candidateText, sex: options.sex || "male", longitude: options.longitude ?? config.defaultLongitude, timezoneOffset: options.timezoneOffset ?? config.timezoneOffset }, config);
...
export function getAnnualPillar(year) {
  ...
  return { year, index, value: STEMS[index % 10].name + BRANCHES[index % 12].name, stem: STEMS[index % 10], branch: BRANCHES[index % 12], naYin: NAYIN[Math.floor(index / 2)] };
}
